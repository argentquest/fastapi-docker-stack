...
class CacheService:
    ...
    async def set(
        ...
        try:
            ...
            logger.debug(f"Cached key '{key}' with expiry of {expires}s.")
            return True
        except Exception as e:
    ...
    async def cache_ai_response(
        ...
        if await self.set(cache_key, payload, expires):
            logger.info(f"Successfully cached AI response with key: {cache_key}")
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            if retrieved != test_value:
                raise ValueError("SET/GET operation mismatch")
