...
def main():
    ...
    if not container_status['all_running']:
        print("[X] FAILED: The following containers are not running: {container_status['missing']}")
        print("Please check `docker-compose ps` and `docker-compose logs`.")
        return False
    print("[OK] All expected containers are running.")
    ...
    if not health_status['healthy']:
        print("[X] FAILED: The application's health endpoint reports issues.")
        print("Unhealthy services reported: {health_status['unhealthy']}")
        return False
    print("[OK] Application health endpoint reports all services are healthy.")

    print("\n" + "=" * 60)
    print("[OK] TEST 01 PASSED: All containers are running and report a healthy status.")
    print("=" * 60)
    return True
...
def check_container_status() -> Dict[str, Any]:
    ...
    try:
        ...
        for container in sorted(expected_containers):
            status_icon = "[OK]" if container in running_containers else "[X]"
...
def test_health_endpoint() -> Dict[str, Any]:
    ...
    try:
        ...
        for service, details in health_data.get('containers', {}).items():
            ...
            status_icon = "[OK]" if service_status == 'healthy' else "[X]"
