...
def main_test_logic():
    ...
    if not check_environment():
        print("[X] FAILED: OPENROUTER_API_KEY is not set or is invalid.")
    ...
    if not test_openrouter_service_health():
        print("[X] FAILED: The application reports that the OpenRouter service is not healthy.")
        return False
    print("[OK] Application reports a healthy connection to OpenRouter.")

    # --- Test 3: AI Response Generation ---
    print("\n3. Testing basic AI response generation...")
    if not test_simple_ai_generation():
        print("[X] FAILED: Could not generate a valid AI response.")
        return False
    print("[OK] AI response generated successfully.")

    # --- Test 4: Error Handling ---
    print("\n4. Testing server error handling for invalid requests...")
    if not test_error_handling():
        print("[X] FAILED: The server did not handle invalid requests correctly.")
        return False
    print("[OK] Server correctly handles invalid requests with a 422 error.")

    print("\n" + "=" * 60)
    print("[OK] TEST 03 PASSED: OpenRouter integration is fully functional.")
    print("=" * 60)
    return True
...
def test_simple_ai_generation() -> bool:
    ...
    try:
        response = requests.post("{TEST_API_URL}/ai-test", json=test_data, timeout=30)
        response.raise_for_status()
        ...
        print("   -> Error during AI generation test: {e}")
        return False
...
def test_error_handling() -> bool:
    ...
    try:
        ...
        print("   -> Error during error handling test: {e}")
        return False
