...
def main_test_logic():
    ...
    try:
        ...
        print("✅ Bucket operations are working correctly.")
        ...
        print("✅ File upload, download, and content verification successful.")
        ...
        if client:
            ...
            print(f"✅ Cleaned up {cleaned_count} test object(s).")

    print("\n" + "=" * 60)
    print("✅ TEST 04 PASSED: MinIO storage is fully functional.")
    print("=" * 60)
    return True
...
def test_bucket_operations(client: Minio) -> bool:
    ...
    if not client.bucket_exists(MINIO_BUCKET):
        print(f"   -> Bucket '{MINIO_BUCKET}' not found. Creating it...")
...
def test_file_operations(client: Minio) -> bool:
    ...
    test_filename = f"{TEST_OBJECT_PREFIX}test-file.txt"
    test_content = f"Hello MinIO! Timestamp: {time.time()}"
...
def cleanup_test_objects(client: Minio) -> int:
    ...
    try:
        ...
        print(f"   -> Warning: Could not clean up objects: {e}")
        return 0
