...
def main_test_logic():
    ...
    try:
        ...
        print("[OK] Redis connection successful (Version: {client.info()['redis_version']}).")
        ...
        print("[OK] Basic SET, GET, and DELETE operations are working correctly.")
        ...
        print("[OK] Key expiration is working correctly.")
        ...
        if client:
            ...
            print("[OK] Cleaned up {cleaned_count} test key(s).")

    print("\n" + "=" * 60)
    print("[OK] TEST 05 PASSED: Redis cache is fully functional.")
    print("=" * 60)
    return True
...
def test_basic_operations(client: redis.Redis) -> bool:
    ...
    test_key = "{TEST_KEY_PREFIX}basic-op"
    test_value = "hello-redis-{time.time()}"
    ...
    if retrieved_value != test_value:
        raise ValueError("Value mismatch: Expected '{test_value}', got '{retrieved_value}'.")
    print("   -> GET '{test_key}' and verified content.")

    # 3. DELETE the key.
    delete_count = client.delete(test_key)
    if delete_count != 1:
        raise ValueError("Expected to delete 1 key, but deleted {delete_count}.")
    print("   -> DELETE '{test_key}'.")

    # 4. Verify the key no longer exists.
    if client.exists(test_key):
        raise ValueError("Key '{test_key}' still exists after deletion.")
...
def test_expiration(client: redis.Redis) -> bool:
    ...
    test_key = "{TEST_KEY_PREFIX}ttl-test"
    ...
    print("   -> SET '{test_key}' with a {expire_time_seconds}s TTL.")
    ...
    print("   -> Verified key no longer exists after {expire_time_seconds+1}s.")
    return True
...
def cleanup_test_data(client: redis.Redis) -> int:
    ...
    cursor = '0'
    while cursor != 0:
        cursor, keys = client.scan(cursor=cursor, match="{TEST_KEY_PREFIX}*", count=100)
